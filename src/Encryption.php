...
class Encryption
{
    ...
    private static function calculateAgreementKey(JWK $private_key, JWK $public_key): string
    {
        ...
        try {
            ...
            return hex2bin(self::addNullPadding(self::$curve->mul($pub_key->getPoint(), $priv_key->getSecret())->getX()->toBase(16), 64)); // @phpstan-ignore-line
        } catch (\Throwable $e) {
    ...
    private static function addNullPadding(string $data, int $length = 32): string
    {
        return str_pad($data, $length, chr(0), STR_PAD_LEFT);
    }
}
